...
Token.prototype.toString = function () {
  return (
    `Token(value='${this.value}', type=${this.type}, ` +
...
function Lexer(src) {
  assert(typeof src === "string", "Expected source string");
  this.src = src;
  this.line = 1;
  this.column = 1;
  this.atError = false;
...
Lexer.prototype.newToken = function (tokenType, value = null) {
  return new Token(
    value !== null ? value : this.src.slice(this.startIndex, this.currentIndex),
    this.line,
    this.column,
    tokenType,
    errors.EL0000
...
Lexer.prototype.eofToken = function () {
  return new Token("", this.line, this.column, tokens.TOKEN_EOF, errors.EL0000);
};
...
Lexer.prototype.getSrcWithPaddingAtLine = function (token) {
  ...
  for (lineEnd = lineStart; lineEnd < this.src.length; lineEnd++) {
    if (
      (this.src[lineEnd] === "\r" && this.src[lineEnd + 1] === "\n") ||
      this.src[lineEnd] === "\n"
    ) {
      break;
    }
...
Lexer.prototype.atEnd = function () {
  return (
    this.currentIndex >= this.src.length || this.src[this.currentIndex] === ""
  );
};
...
function isFloat(str) {
  ...
  if (isDigit(str[currIdx]) || str[currIdx] === ".") {
    ...
    while (isDigit(str.charAt(currIdx))) {
      // Digit*
      currIdx++;
      digitCount++;
    }
    if (str.charAt(currIdx).toLowerCase() === "e") {
      // Exp?
      // check:
      currIdx++;
      hasExp = 1;
      if (str.charAt(currIdx) === "+" || str.charAt(currIdx) === "-") {
        currIdx++;
        signCount++;
      }
      while (isDigit(str.charAt(currIdx))) {
    ...
    return digitCount + signCount + hasExp + hasPoint === str.length;
  }
  return 0;
}
...
Lexer.prototype.getToken = function () {
  ...
  switch (char) {
    ...
    case ".":
      ...
      if (isDigit(p)) {
        ...
        return this.newToken(
          this.check(".") ? tokens.TOKEN_DOT_DOT_DOT : tokens.TOKEN_DOT_DOT
        );
      }
    ...
    case "&":
      return this.newToken(
        this.check("=") ? tokens.TOKEN_AND_EQUAL : tokens.TOKEN_BITWISE_AND
      );
    case "!":
      return this.newToken(
        this.check("=") ? tokens.TOKEN_NOT_EQUAL : tokens.TOKEN_EXC_MARK
      );
    case "|":
      return this.newToken(
        this.check("=")
          ? tokens.TOKEN_OR_EQUAL
          : this.check(">")
          ? tokens.TOKEN_PIPE
          : tokens.TOKEN_BITWISE_OR
      );
    case "^":
      return this.newToken(
        this.check("=") ? tokens.TOKEN_XOR_EQUAL : tokens.TOKEN_BITWISE_XOR
      );
    case "=":
      return this.newToken(
        this.check("=")
          ...
          : this.check(">")
          ? tokens.TOKEN_FAT_ARROW
          : tokens.TOKEN_EQUAL
      );
    case "%":
      return this.newToken(
        this.check("=") ? tokens.TOKEN_MOD_EQUAL : tokens.TOKEN_MOD
      );
    case "/":
      return this.newToken(
        this.check("=") ? tokens.TOKEN_DIV_EQUAL : tokens.TOKEN_F_SLASH
      );
    case "+":
      return this.newToken(
        this.check("+")
          ? tokens.TOKEN_PLUS_PLUS
          : this.check("=")
          ? tokens.TOKEN_PLUS_EQUAL
          : tokens.TOKEN_PLUS
      );
    // bitwise
    case "<":
      return this.newToken(
        this.check("<")
          ? this.check("=")
            ? tokens.TOKEN_LSHIFT_EQUAL
            : tokens.TOKEN_BITWISE_LSHIFT
          : this.check("=")
          ? tokens.TOKEN_LESS_THAN_EQUAL
    ...
    case ">":
      return this.newToken(
        this.check(">")
          ? this.check("=")
            ? tokens.TOKEN_RSHIFT_EQUAL
            : tokens.TOKEN_BITWISE_RSHIFT
          : this.check("=")
          ? tokens.TOKEN_GREATER_THAN_EQUAL
          : tokens.TOKEN_GREATER_THAN
      );
    case "-":
      return this.newToken(
        this.check("-")
          ? tokens.TOKEN_MINUS_MINUS
          : this.check(">")
          ? tokens.TOKEN_ARROW
          : this.check("=")
          ? tokens.TOKEN_MINUS_EQUAL
          : tokens.TOKEN_MINUS
      );
    case "*":
      return this.newToken(
        this.check("*")
          ? this.check("=")
            ? tokens.TOKEN_STAR_STAR_EQUAL
            : tokens.TOKEN_STAR_STAR
          : this.check("=")
          ? tokens.TOKEN_STAR_EQUAL
          : tokens.TOKEN_STAR
      );
    default:

...
RError[EP0008] = createError(EP0008, "Token found mismatches expected token.");

RError[EP0009] = createError(
  EP0009,
  "star arm mixed usage",
  "Consider placing star in a separate arm."
);

RError[EP0010] = createError(
  EP0010,
  "Expected ':' after expression.",
  "Consider adding a ':' after the expression."
);

RError[EP0011] = createError(
  EP0011,
  "case expression cannot be empty.",
  "Consider adding 'of' arms."
);

RError[EP0012] = createError(
  EP0012,
  "Expected ']'",
  "Consider closing with a ']'."
);

RError[EP0013] = createError(
  EP0013,
...
RError[EP0016] = createError(
  EP0016,
  "star arm not last",
  "star arm must be the last arm in a case expression, when used."
);

RError[EP0017] = createError(
  EP0017,
  "Expected ')'",
  "Consider closing with a ')'"
);

RError[EP0018] = createError(
  EP0018,
...
RError[EP0046] = createError(
  EP0046,
  "Illegal block .",
  `Expected a block of code.${SEP}` + `Consider creating a valid block of code.`
);

RError[EP0047] = createError(
  EP0047,
  "Invalid assignment target.",
  "Can't assign to 'ref'."
);

RError[EP0048] = createError(
  EP0048,

 ...
 * @returns {Map<StringObject, Value>}
 */
...
Value.prototype.stringify = function (includeQuotes = false, rvm = null) {
  switch (this.type) {
    ...
    case VAL_STRING: {
      ...
      return includeQuotes ? `'${this.asString().raw}'` : this.asString().raw;
    }
    case VAL_LIST:
    ...
    case VAL_INSTANCE: {
      ...
      if (
        rvm &&
        (strMethod = this.asInstance().def.getMethod(rvm.stringMethodName))
      ) {
        // push instance
        rvm.pushStack(this);
...
Value.prototype.listEquals = function listEquals(other) {
  if (this.value.elements.length !== other.value.elements.length) return false;
  else if (this.value === other.value) return true;
  for (let i = 0; i < other.value.elements.length; i++) {
    if (!this.value.elements[i].equals(other.value.elements[i])) return false;
  }
  return true;
};

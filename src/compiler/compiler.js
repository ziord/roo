...
class Compiler extends ast.NodeVisitor {
  constructor(parser = null, module = null, fnType = ast.FnTypes.TYPE_SCRIPT) {
    super();
    this.parser = parser;
  ...
  visitNumberNode(node) {
    ...
    gen.emitConstant(this.fn.code, new vmod.Value(type, node.value), node.line);
  }
  ...
  visitShowNode(node) {
    ...
    gen.emitBytes(this.fn.code, opcode.$SHOW, node.nodes.length, node.line);
  }
  ...
  visitVarNode(node) {
    ...
    if ((index = this.findLocal(node)) !== -1) {
      ...
      gen.emit2BytesOperand(this.fn.code, opcode.$GET_LOCAL, index, node.line);
    }
    // check if it's an upvalue
    else if ((index = this.findUpvalue(node)) !== -1) {
      // only 1 byte (256) upvalues allowable for now
      gen.emitBytes(this.fn.code, opcode.$GET_UPVALUE, index, node.line);
    }
    // might be a global
    else {
      index = this.storeString(node.name);
      gen.emit2BytesOperand(this.fn.code, opcode.$GET_GLOBAL, index, node.line);
    }
  }
  ...
  visitOrExprNode(node) {
    ...
    const conditionEnd = gen.emitJump(this.fn.code, opcode.$JUMP, node.line);
  ...
  visitIfElseNode(node) {
    ...
    const elseJmp = gen.emitJump(this.fn.code, opcode.$JUMP_IF_FALSE_OR_POP);
  ...
  visitWhileLoopNode(node) {
    ...
    // or pop condition result off the stack if it evaluates to true
    const exitJmp = gen.emitJump(this.fn.code, opcode.$JUMP_IF_FALSE_OR_POP);
  ...
  visitForLoopNode(node) {
    ...
    if (node.conditionExpr) {
      this.visit(node.conditionExpr);
      bodyJmp = gen.emitJump(this.fn.code, opcode.$JUMP_IF_FALSE_OR_POP);
    }
    if (node.incrExpr) {
    ...
    if (node.initExpr) {
      ...
      if (node.initExpr.type === ast.ASTType.AST_NODE_VAR_DECL) {
        ...
        gen.emit2BytesOperand(this.fn.code, opcode.$POP_N, localsCount);
  ...
  visitForInLoopNode(node) {
    ...
    const conditionExprNode = new ast.AndExprNode(assignNode, unaryNode, line);
  ...
  assertJumpOffset(offset, controlNode) {
    if (offset > utils.UINT16_MAX) {
      this.compilationError("code body too large to jump over", controlNode);
    }
  }
  ...
  defineVariable(name, index, line) {
    if (this.currentScope > 0) {
      ...
      gen.emit2BytesOperand(this.fn.code, opcode.$DEFINE_GLOBAL, index, line);
    }
  }
  ...
  compileDerefMethodCall(node) {
    ...
    gen.emit2BytesOperand(
      this.fn.code,
      node.hasSpread ? opcode.$INVOKE_DEREF_UNPACK : opcode.$INVOKE_DEREF,
      index,
      node.line
    );
  ...
  visitDefNode(node) {
    ...
    if (node.derivingNode) {
      ...
      gen.emitByte(this.fn.code, opcode.$DERIVE, node.derivingNode.line);
    }
  ...
  compileFromImport(node) {
    ...
    node.names.forEach(({ nameVar, aliasVar }) => {
      // VarNode(), VarNode()
      const expr = new ast.DotExprNode(moduleVar, nameVar, nameVar.line);
      const decl = new ast.VarDeclNode(
        aliasVar.name,
        expr,
        false,
        aliasVar.line
  ...
  transformSpreadNode(node) {
    ...
    const ifBlock = new ast.BlockNode([new ast.ReturnNode(param, line)], line);
  ...
  visitTryNode(node) {
    ...
    if (node.elseBlock) {
      ...
      // the except block
      const elseJmp = gen.emitJump(this.fn.code, opcode.$JUMP, node.line);
  ...
  visitDelNode(node) {
    ...
    if (node.isSubscript) {
      ...
      this.fn.code.bytes[this.fn.code.length - 1] = opcode.$DELETE_SUBSCRIPT;
    } else {
      ...
      if (bytecode === opcode.$GET_DEREF_PROPERTY) {
        return this.compilationError("Cannot delete deref property", node);
      }
      this.fn.code.bytes[this.fn.code.length - 3] = opcode.$DELETE_PROPERTY;
    }
  }
